 ...
 * The cache is invalidated on non-initial sync_reset and refreshed on new snapshots.
 */
import type { SyncTables } from "../domain/types";

const DB_NAME = "shedflare-money-offline";
const DB_VERSION = 1;
  ...
  try {
    ...
    return new Promise((resolve, reject) => {
      ...
      request.onsuccess = () => {
        ...
        if (!result?.tables || typeof result?.lastServerSeq !== "number") {
          resolve(null);
          return;
        }
        resolve({
          tables: result.tables as SyncTables,
