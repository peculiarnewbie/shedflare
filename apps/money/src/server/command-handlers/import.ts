...
export function handleImportCommands(c: string, p: any, a: DataAccess): CR {
  ...
  if (!p.accountId || txs.length === 0) return { ok: true, data: { added: 0, updated: 0, errors: [] } };
    ...
    try {
      ...
      if (row.importedDescription) {
        const existing = a.queryOne<{ id: string }>(
          "SELECT id FROM transactions WHERE imported_description = ?", row.importedDescription,
        );
        if (existing) {
          a.exec(
            "UPDATE transactions SET amount = ?, payee = ?, notes = ?, date = ?, category_id = ?, updated_at = ? WHERE id = ?",
            row.amount, row.payee, row.notes, row.date, row.categoryId, now, existing.id,
          );
          updated++; continue;
        }
      }
