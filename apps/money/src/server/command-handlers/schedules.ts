...
export function handleScheduleCommands(
  ...
  switch (commandType) {
    ...
    case "update_schedule": {
      ...
      if (payload.fields.name !== undefined) {
        fields.push("name = ?");
        params.push(payload.fields.name);
      }
      if (payload.fields.accountId !== undefined) {
        fields.push("account_id = ?");
        params.push(payload.fields.accountId);
      }
      if (payload.fields.payeeId !== undefined) {
        fields.push("payee_id = ?");
        params.push(payload.fields.payeeId);
      }
      if (payload.fields.categoryId !== undefined) {
        fields.push("category_id = ?");
        params.push(payload.fields.categoryId);
      }
      if (payload.fields.amount !== undefined) {
        fields.push("amount = ?");
        params.push(payload.fields.amount);
      }
      if (payload.fields.recurrenceRules !== undefined) {
        fields.push("recurrence_rules = ?");
        params.push(payload.fields.recurrenceRules);
      }
    ...
    case "skip_schedule_date": {
      access.exec(
        "UPDATE schedules SET next_date = ?, updated_at = ? WHERE id = ?",
        payload.nextDate,
    ...
    case "post_schedule_transaction": {
      access.exec(
        "UPDATE schedules SET completed = 1, updated_at = ? WHERE id = ?",
        new Date().toISOString(),
        payload.scheduleId,
      );
