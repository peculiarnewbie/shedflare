  ...
  switch (commandType) {
    ...
    case "update_category": {
      ...
      if (payload.hidden !== undefined) {
        fields.push("hidden = ?");
        params.push(payload.hidden ? 1 : 0);
      }
      if (payload.groupId !== undefined) {
        fields.push("group_id = ?");
        params.push(payload.groupId);
      }
      if (payload.goalDef !== undefined) {
        fields.push("goal_def = ?");
        params.push(payload.goalDef);
      }
    ...
    case "update_category_group": {
      ...
      if (payload.isIncome !== undefined) {
        fields.push("is_income = ?");
        params.push(payload.isIncome ? 1 : 0);
      }
      params.push(payload.id);
