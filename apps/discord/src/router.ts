...
import { GATEWAY_WEBHOOK_HEADER, GatewayWebhookEventSchema } from "#/gateway/protocol";
import { getGatewayStub } from "#/gateway/stub";
import { handleGatewayWebhookEvent } from "#/handlers/message-create";
import { parse } from "valibot";

function json<Data>(data: Data, status = 200): Response {
...
export async function routeRequest(
  ...
  if (url.pathname === "/internal/gateway" && request.method === "POST") {
    ...
    const event = parse(GatewayWebhookEventSchema, await request.json());
