...
type ClassifiedExaError = { error: string; hint: string };

function classifyExaError(error: Error): ClassifiedExaError {
  ...
  return { error: error.message.slice(0, 200), hint: "Unknown search error." };
}
...
export function createSearchExecutor(input: {
  ...
  return async function runExaWebSearch(args: {
    ...
    try {
      ...
      const { error: message, hint } = classifyExaError(caught);
